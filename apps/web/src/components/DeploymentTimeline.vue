<script setup lang="ts">
import { computed } from 'vue'
import {
  Play,
  Terminal,
  Upload,
  Trash2,
  PackageOpen,
  Save,
  CheckCircle2,
  AlertCircle,
  AlertTriangle,
  RotateCcw,
  Wrench,
  Loader2,
} from 'lucide-vue-next'
import type { TimelineEvent, TimelineKind } from '../utils/deployment-timeline'

const props = defineProps<{
  events: TimelineEvent[]
}>()

const emit = defineEmits<{
  (e: 'jump', rawLineIndex: number): void
}>()

const iconMap: Record<TimelineKind, any> = {
  start: Play,
  'pre-deploy': Terminal,
  push: Upload,
  clean: Trash2,
  extract: PackageOpen,
  'post-deploy': Terminal,
  archive: Save,
  gc: Trash2,
  'manual-mark': Wrench,
  'rollback-start': RotateCcw,
  'rollback-pre': Terminal,
  'rollback-clean': Trash2,
  'rollback-extract': PackageOpen,
  'rollback-post': Terminal,
  success: CheckCircle2,
  failed: AlertCircle,
  running: Loader2,
}

function toneTextClass(tone?: TimelineEvent['tone']): string {
  switch (tone) {
    case 'success': return 'text-success'
    case 'danger': return 'text-danger'
    case 'warning': return 'text-yellow-400'
    case 'primary': return 'text-primary'
    default: return 'text-textMuted'
  }
}

const items = computed(() => props.events)

function fmtTime(ms?: number): string {
  if (ms === undefined) return '—'
  const d = new Date(ms)
  return d.toLocaleTimeString([], { hour12: false }) + '.' + String(d.getMilliseconds()).padStart(3, '0')
}

function fmtDuration(ms?: number): string {
  if (ms === undefined) return ''
  if (ms < 1000) return `${ms} ms`
  const s = ms / 1000
  if (s < 60) return `${s.toFixed(1)}s`
  const m = Math.floor(s / 60)
  const rs = (s - m * 60).toFixed(0)
  return `${m}m ${rs}s`
}

function handleJump(ev: TimelineEvent) {
  if (ev.rawLineIndex === undefined) return
  emit('jump', ev.rawLineIndex)
}
</script>

<template>
  <div class="space-y-1.5">
    <div v-if="items.length === 0" class="text-textMuted text-sm py-8 text-center font-sans">
      暂无时间线事件
    </div>
    <template v-else>
      <div
        v-for="(ev, idx) in items"
        :key="idx"
        class="flex items-center gap-2.5"
      >
        <component
          :is="iconMap[ev.kind]"
          class="shrink-0 w-4 h-4"
          :class="[toneTextClass(ev.tone), ev.kind === 'running' ? 'animate-spin' : '']"
        />
        <button
          type="button"
          class="flex-1 min-w-0 text-left bg-panel/40 border border-border/60 hover:border-primary/40 hover:bg-panel/70 rounded-md px-3 py-2 text-xs transition-colors font-sans"
          :class="ev.rawLineIndex === undefined ? 'cursor-default' : 'cursor-pointer'"
          @click="handleJump(ev)"
        >
          <div class="flex items-center gap-2 flex-wrap">
            <span class="font-medium text-textMain">{{ ev.label }}</span>
            <span
              v-if="ev.exitCode !== undefined"
              class="px-1 py-0 rounded text-[10px] font-mono"
              :class="ev.exitCode === 0 ? 'bg-success/10 border border-success/30 text-success' : 'bg-danger/10 border border-danger/30 text-danger'"
            >exit {{ ev.exitCode }}</span>
            <span class="text-textMuted/60 font-mono text-[10px]">{{ fmtTime(ev.startedAt) }}</span>
            <template v-if="ev.endedAt !== undefined && ev.endedAt !== ev.startedAt">
              <span class="text-textMuted/40 font-mono text-[10px]">→</span>
              <span class="text-textMuted/60 font-mono text-[10px]">{{ fmtTime(ev.endedAt) }}</span>
            </template>
            <span
              v-if="ev.durationMs !== undefined"
              class="text-textMuted font-mono ml-auto text-[10px]"
            >{{ fmtDuration(ev.durationMs) }}</span>
          </div>
          <div
            v-if="ev.detail"
            class="mt-1 text-textMuted font-mono text-[11px] truncate"
            :title="ev.detail"
          >{{ ev.detail }}</div>
          <div
            v-if="ev.estimated"
            class="mt-1 inline-flex items-center gap-0.5 text-yellow-400/70 text-[10px] font-mono"
            title="时间戳为按日志行号比例估算"
          >
            <AlertTriangle class="w-3 h-3" />
            仅供参考·点击跳转原始日志
          </div>
        </button>
      </div>
    </template>
  </div>
</template>
